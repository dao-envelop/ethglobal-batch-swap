...
contract ETHGlobalSmartWallet is Wallet, ERC721Upgradeable, IERC4906 {
    ...
    function __ETHGlobalSmartWallet_init(InitParams calldata _init) internal onlyInitializing fixEtherBalance {
    ...
    function __ETHGlobalSmartWallet_init_unchained(InitParams calldata _init) internal onlyInitializing {
     ...
     */
    function createWalletOnFactory(InitParams memory _init) public virtual notDelegated returns (address wallet) {
        wallet = IWalletFactory(FACTORY).createWallet(address(this), abi.encodeWithSignature(INITIAL_SIGN_STR, _init));
    }
     ...
     * @param _init  see `struct InitParams` above.
     */
    function createWalletOnFactory2(InitParams memory _init) public virtual notDelegated returns (address wallet) {
        bytes32 salt = keccak256(abi.encode(address(this), msg.sender, ++nonce[msg.sender]));
        wallet =
            IWalletFactory(FACTORY).createWallet(address(this), abi.encodeWithSignature(INITIAL_SIGN_STR, _init), salt);
    }
    ...
    ) external onlyWalletKeeper returns (bytes[] memory r) {
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return string(
            abi.encodePacked(DEFAULT_BASE_URI, block.chainid.toString(), "/", uint160(address(this)).toHexString(), "/")
        );
    }
